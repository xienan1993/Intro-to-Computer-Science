...
def create_data_structure(string_input):
    ...
    for sentence in sentences:
        if sentence:
            ...
            if not subject in network:
                network[subject] = {}
            if "is connected to" in sentence:
                network[subject]['connections'] = []
                _, people = sentence.split('is connected to')
                for person in people.split(','):
                    network[subject]['connections'].append(person.lstrip())
            else:
                network[subject]['games'] = []
                _, games = sentence.split('likes to play')
                for game in games.split(','):
                    network[subject]['games'].append(game.lstrip())
    return network
...
def get_secondary_connections(network, user):
    if user not in network:
        ...
        for connection in network[user]['connections']:
                secondary = secondary + [connect for connect in network[connection]['connections'] if connect not in secondary]
...
def count_common_connections(network, user_A, user_B):
    if not user_A in network or not user_B in network:
        return False
    common = 0
    a_connections = get_connections(network, user_A)
    b_connections = get_connections(network, user_B)
    if a_connections and b_connections:
